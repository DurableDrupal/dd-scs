...
router.put('/books', function(req, res) {
    ...
    // console.log("unBook", unBook)
    unBook.bookBody.value = req.body.content
    // skip autor for now, since we would have to look that up :(
        // no, now we got it!
    unBook.bookAuthor.push(req.body.data.autor)    
    unBook.metaData.itemName = req.body.data.titulo
    unBook.metaData.itemSlug = req.body.data.slug
    unBook.metaData.publishedDate = req.body.data.fecha
    unBook.metaData.published = true
    // console.log('the new unBook: ', unBook)

    var query = {
        'metaData.itemSlug': req.body.data.slug
    }
    Book.findOneAndUpdate(query, unBook, {upsert: true, new: true},
      function(err, book) {
        if (err)
            return res.json({
